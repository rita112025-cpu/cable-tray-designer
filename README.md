...
- Auto* 產生的新元件會繼承高度：Tee / Cross 與拆出的主線沿用 Main；Elbow / Reducer / 分支 Reducer 兩側高度相同則沿用，任一側未知則為未知；兩側都有值但不同時，v5.9 起會被連接驗證擋下（見下）。
...
`CT.validateConnections` 新增「高度一致」檢查（與「高程一致」是各自獨立的檢查）：

| 連接兩側的 `trayHeight` | 結果 |
| --- | --- |
| 都有值且相同 | Valid |
| 都有值但不同 | **Warning**（不是 Invalid） |
| 任一側未設定 | 不判定，不出現這項檢查，也不 Warning |

- 用 Warning 而非 Invalid：不同高度的 Tray 不一定不能接（實務上可能有變高接頭、轉接件），但目前模型沒有這類 fitting，
  所以只提示，**不嘗試自動解決高度轉接**。
- Reducer 目前只有單一 `trayHeight`，連接比較的是兩個 block 各自的高度；還無法表達「兩端高度不同」的變徑。
- Auto* 會自然受到檢查（不需改 transaction）：新增的連接必須全為 Valid，所以高度不一致的兩條 Tray 之間，
  Auto Elbow / Reducer / Tee / Cross 的提案會被擋下並說明「Tray Height 不一致」；原本就有高度 Warning 的既有連接，
